    ...
    public async Task<ResponseBase<Product?>> GetBySlugAsync(GetProductBySlugRequest request)
    {
        try
        {
            ...
            return product is null
                ? new ResponseBase<Product?>(null, 404, "Produto não encontrado")
                : new ResponseBase<Product?>(product);
        }
        catch
        {
            return new ResponseBase<Product?>(null, 500, "Não foi possível recuperar o produto");
        }
    }
}
